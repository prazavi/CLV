...
# this function is for rounding up
...
# This block will create a list if time id's
...
# This block is for calculating CLV and other data for each customer
    ...
    if customer_id==int(df_sales['customer_id'][i]):
        for j in range(len(timeList)):
            if int(df_sales['time_id'][i]) in timeList[j]:
                coval=24-j
        tavan=(-1)*(coval-0.5)
        clv+=truncate((float(df_sales['store_sales'][i])-float(df_sales['store_cost'][i]))/(1.01**tavan),2)
    else:
...
workbook = xlsxwriter.Workbook('clv_data_rounded.xlsx')
